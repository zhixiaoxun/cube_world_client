...
public class AmplifyOcclusionBase : MonoBehaviour
{
	...
	[Range( 0, 16 )] public float PowerExponent = 1.4f;
	...
	{
        if ( m_occlusionMat == null )
			m_occlusionMat = new Material(ResourceManager.Instance.LoadShader("Shaders/SSAO/Occlusion")) { hideFlags = HideFlags.DontSave };
		if ( m_blurMat == null )
			m_blurMat = new Material(ResourceManager.Instance.LoadShader("Shaders/SSAO/Blur")) { hideFlags = HideFlags.DontSave };
		if ( m_copyMat == null )
			m_copyMat = new Material(ResourceManager.Instance.LoadShader("Shaders/SSAO/Copy")) { hideFlags = HideFlags.DontSave };
	}
	...
	void CommandBuffer_FillComputeOcclusion( CommandBuffer cb, TargetDesc target )
	{
		...
		if ( m_occlusionRT == null || m_occlusionRT.width != target.width || m_occlusionRT.height != target.height || !m_occlusionRT.IsCreated() )
		{
            ...
            if (SystemInfo.SupportsRenderTextureFormat(RenderTextureFormat.RGHalf))
                ...
                m_occlusionRT = SafeAllocateRT( "_AO_OcclusionTexture", target.width, target.height, RenderTextureFormat.RGFloat, RenderTextureReadWrite.Linear );
            else if(SystemInfo.SupportsRenderTextureFormat(RenderTextureFormat.RG32))
                m_occlusionRT = SafeAllocateRT("_AO_OcclusionTexture", target.width, target.height, RenderTextureFormat.RG32, RenderTextureReadWrite.Linear);
            else if(SystemInfo.SupportsRenderTextureFormat(RenderTextureFormat.RG16))
                m_occlusionRT = SafeAllocateRT("_AO_OcclusionTexture", target.width, target.height, RenderTextureFormat.RG16, RenderTextureReadWrite.Linear);
            else if(SystemInfo.SupportsRenderTextureFormat(RenderTextureFormat.RGInt))
                m_occlusionRT = SafeAllocateRT("_AO_OcclusionTexture", target.width, target.height, RenderTextureFormat.RGInt, RenderTextureReadWrite.Linear);
            else
                m_occlusionRT = SafeAllocateRT("_AO_OcclusionTexture", target.width, target.height, RenderTextureFormat.Default, RenderTextureReadWrite.Linear);
        }
