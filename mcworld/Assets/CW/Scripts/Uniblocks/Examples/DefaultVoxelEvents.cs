...
public class DefaultVoxelEvents : VoxelEvents {
    ...
    public override void OnMouseUp(int mouseButton, VoxelInfo voxelInfo)
    {
            if (mouseButton == 0)
            { // destroy a block with LMB
                ...
                if (blockSelection != null)
                {
                    blockSelection.GetComponent<WreckBlockLogic>().StopWreck(voxelInfo);
                }
            }
    }
    ...
    public override void OnLook ( VoxelInfo voxelInfo ) {
		...
		if (blockSelection != null) {
            ...
            blockSelection.GetComponent<WreckBlockLogic>().OnLook(voxelInfo);
        }

        GameObject crosshairs = GameObject.Find("Canvas/Crosshairs");
        if (crosshairs != null) { 
