...
namespace Uniblocks
{
    ...
    public class ChunkMeshCreator : MonoBehaviour
    {
        ...
        private void CreateCustomPrefab(ushort voxel, int x, int y, int z, GameObject prefab)
        {
            Index index = new Index(x, y, z);
            chunk.LogicBlocks[index] = Instantiate(prefab, this.transform);
            chunk.LogicBlocks[index].transform.position = chunk.ChunkIndex.ToVector3()*SideLength + index.ToVector3() + new Vector3(0, -0.5f, 0);
        }
        private void CreateCustomMesh(ushort voxel, int x, int y, int z, Mesh mesh)
        {
            // check if mesh exists, if it doesn't exist, it is an empty block
            if (mesh == null)
            {
                //Don't print, it's too slow.
                //Debug.LogWarning("Uniblocks: The voxel id " + voxel + " uses a custom mesh, but no mesh has been assigned! There will be am empty block.");
                return;
            }
