...
public class EngineSettings : EditorWindow {
	...
	public void OnGUI () {
		...
		if ( FindEngine() == false ) {
			...
			GUILayout.Space (20);
			GUILayout.Label ("Multiplayer & Internetplayer");
			engine.lEnableMultiplayer = EditorGUILayout.Toggle ( "Enable multiplayer", engine.lEnableMultiplayer );
            if(engine.lEnableMultiplayer)
                engine.lEnableInternetplayer = !engine.lEnableMultiplayer;
            engine.lEnableInternetplayer = EditorGUILayout.Toggle("Enable internetplayer", engine.lEnableInternetplayer);
            if (engine.lEnableInternetplayer)
                engine.lEnableMultiplayer = !engine.lEnableInternetplayer;
